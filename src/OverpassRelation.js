...
class OverpassRelation extends OverpassObject {
  updateData (data, options) {
    ...
    if (data.geometryMembers) {
      this.geometryMembers = data.geometryMembers
      this.properties |= OverpassFrontend.GEOM
    }
  ...
  updateGeometry () {
    ...
    if (!this.bounds || (this.properties & OverpassFrontend.BBOX) === 0) {
    ...
    if (!this.center || (this.properties & OverpassFrontend.CENTER) === 0) {
      this.center = this.bounds.getCenter()
      this.properties = this.properties | OverpassFrontend.BBOX | OverpassFrontend.CENTER
    }
  }
  ...
  GeoJSON (options = { meta: true, geom: true }) {
    ...
    if (options.geom && this.geometryMembers) {
      if (this.overpass.separateSkelGeom) {
        const tmp = {
          elements: [{
            ...
            id: this.osm_id,
            tags: this.tags,
            members: this.geometryMembers.map((m, i) => {
              return { ...m, ref: i }
            })
          }]
        }
        const resolved = osmtogeojson(tmp)
  ...
  _outXml (options, document, result) {
    ...
    if (this.members && ((!options.ids && !options.tags) || options.body || options.skel)) {
      ...
      if (options.geom && this.overpass.separateSkelGeom) {
        this.geometryMembers.forEach(member => {
          const blank = document.createTextNode('\n  ')
          result.appendChild(blank)
